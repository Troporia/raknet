...
use crate::util::packet_id::ALREADY_CONNECTED;
...
impl RakCodec for AlreadyConnected {
    ...
    fn deserialize<R: Read>(reader: &mut R) -> Result<Self, RakCodecError> {
        ...
        if id != ALREADY_CONNECTED {
            return Err(RakCodecError::UnexpectedPacketID(ALREADY_CONNECTED, id));
        }
